...
class PlayerTargetAdmin(admin.ModelAdmin):
    ...
    def show_name(self, obj):
        return format_html(
            "<a target='_blank' rel='noopener' href='http://{server}.fourmizzz.fr/Membre.php?Pseudo={name}'>{name}</a>",
            server=obj.server,
            name=obj.name,
        )
...
class AllianceTargetAdmin(admin.ModelAdmin):
    ...
    def show_name(self, obj):
        return format_html(
            "<a target='_blank' rel='noopener' href='http://{server}.fourmizzz.fr/classementAlliance.php?alliance={name}'>{name}</a>",
            server=obj.server,
            name=obj.name,
        )
...
class RankingSnapshotAdmin(admin.ModelAdmin):
    ...
    def show_player_name(self, obj):
        return format_html(
            "<a target='_blank' rel='noopener' href='http://{server}.fourmizzz.fr/Membre.php?Pseudo={player}'>{player}</a>",
            server=obj.server.name,
